...
1. start from the same public TinyStories-8M base checkpoint;
2. create a controlled A-only post-training checkpoint;
3. expose only the post-trained checkpoint to the scanner;
...
6. reopen only this post-only selected set under an entry-distribution KL budget of 0.05;
7. use the base model only afterward for oracle evaluation.

The table therefore measures **operational reopening under a fixed post-only heuristic**, not exact reconstruction.
...
- Deep erosion cannot be identified reliably from semantic plausibility alone. In the deep SFT=20 controls, candidates can still look natural and even be selected, while oracle recovery under the same KL budget is small. The median recovery of the three deep-eroded conditions here is only about **2.4%**.
- A coherent candidate being easy to strengthen does not prove that it is a historically preserved pre-post-training route.
- We do not currently recover exact pretrained probabilities, training history, or arbitrary suppressed capabilities.
